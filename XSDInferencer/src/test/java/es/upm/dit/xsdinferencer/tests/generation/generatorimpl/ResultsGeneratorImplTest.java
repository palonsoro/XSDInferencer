...
public class ResultsGeneratorImplTest {
	 ...
	 * Mock XSD generator
	 */
	private SchemaDocumentGenerator<Document> mockXSDDocumentGeneratorTest;

	/**
	 * Mock XSD generator
	 */
	private SchemaDocumentGenerator<Document> mockXSDDocumentGeneratorTesting;
	...
	/**
	 * JDOM2 {@link Document} with the contents of the resulting XSD for the namespace 'testing'
	 */
	private Document resultingXSDNSTesting;

	/**
	 * Mock {@link SchemaDocumentGeneratorFactory} so that the desired mock XSDs are generated by the tested implementation.
	 */
	private SchemaDocumentGeneratorFactory mockXSDDocumentGeneratorFactory;
	...
	public void setUp() throws Exception{
		...
		resultingXSDNSTestingString = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"+ lineSeparator +
				"<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" xmlns:test=\"http://prueba.net\" xmlns:testing=\"http://probando.net\" elementFormDefault=\"qualified\" />" + lineSeparator +"<!-- I am testing -->" + lineSeparator;
		...
		when(configuration.getWorkingFormat()).thenReturn("xml");
		...
		mockXSDDocumentGeneratorTest = mock(SchemaDocumentGenerator.class);
		when(mockXSDDocumentGeneratorTest.generateSchemaDocument(eq(schema), eq(configuration)))
		.thenReturn(resultingXSDNSTest);

		mockXSDDocumentGeneratorTesting = mock(SchemaDocumentGenerator.class);
		when(mockXSDDocumentGeneratorTesting.generateSchemaDocument(eq(schema), eq(configuration)))
		.thenReturn(resultingXSDNSTesting);

		mockXSDDocumentGeneratorFactory = mock(SchemaDocumentGeneratorFactory.class);
		when(mockXSDDocumentGeneratorFactory.getXMLSchemaDocumentGeneratorInstance(eq(NAMESPACE_URI_TEST),anyString(),any(XSDFileNameGenerator.class))).thenReturn(mockXSDDocumentGeneratorTest);
		when(mockXSDDocumentGeneratorFactory.getXMLSchemaDocumentGeneratorInstance(eq(NAMESPACE_URI_TESTING),anyString(),any(XSDFileNameGenerator.class))).thenReturn(mockXSDDocumentGeneratorTesting);
	...
	public void testNormal() throws XSDConfigurationException{
		ResultsGenerator resultsGenerator = new ResultsGeneratorImpl(mockXSDDocumentGeneratorFactory, mockStatisticResultsDocGeneratorFactory, mockXsdFileNameGenerator);
		Results results = resultsGenerator.generateResults(schema, configuration);
		...
		Map<String, String> schemasAsXMLStrings = results.getXSDsAsStrings();
		...
		Map<String, String> statisticsAsXMLStrings = results.getStatisticsAsStrings();
